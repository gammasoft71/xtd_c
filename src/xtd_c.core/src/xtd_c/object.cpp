...
#include "../include/xtd_c/object.h"
  ...
  const char* xtd_object_to_string(const xtd_object* value) {
    if (value == nullptr) {
      assert_(value != nullptr, "value is null");
  ...
  size_t xtd_object_to_string_s(const xtd_object* value, char* string, size_t size) {
   if (value == nullptr || string == nullptr || size == 0) {
      assert_(value != nullptr, "value parameter is null");
      assert_(string != nullptr, "string parameter is null");
      assert_(size != 0, "value pramater is equal to 0");
      errno = EINVAL;
      return 0;
    }
    return to_c_string_s(string, size, reinterpret_cast<const object*>(value)->to_string());
  }
}
