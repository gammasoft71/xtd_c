...
int main(int argc, char* argv[]) {
  ...
  xtd_object_destroy(XTD_OBJECT_PTR(str));
  ...
  size_t size = xtd_object_to_string(XTD_OBJECT_PTR(my_object), NULL, 0);
  char* string = (char*)malloc(size);
  xtd_object_to_string(XTD_OBJECT_PTR(my_object), string, size);
  printf("%s\n", string);
  free(string);
